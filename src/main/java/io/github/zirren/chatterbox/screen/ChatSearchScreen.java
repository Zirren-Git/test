...
public class ChatSearchScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		String count = I18n.get("chatterbox.search.results", resultCount);
		graphics.text(this.font, count, this.width / 2 + 134, 15, 0xA0A0A0, false);
		if (rows.isEmpty()) {
			String empty = I18n.get(searchBox != null && !searchBox.getValue().isEmpty()
	...
	private class ResultList extends AbstractSelectionList<ResultList.Entry> {
		...
		private class ResultEntry extends Entry {
			...
			@Override
			public Component getNarration() {
				return Component.literal(result.text);
			}
